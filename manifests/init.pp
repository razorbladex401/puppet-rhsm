...
class rhsm (
  ...
  if $repo_tools {
    exec { 'RHSM-REPO-TOOLS':
      ...
      onlyif    => 'subscription-manager repos --list-disabled | grep server-satellite-tools',
      path      => '/bin:/usr/bin:/usr/sbin',
      logoutput => true,
      require   => Exec['RHSM-register'],
    }
  ...
  if $repo_els {
    exec { 'RHSM-REPO-ELS':
      ...
      onlyif  => 'subscription-manager repos --list-disabled | grep server-els-rpms',
      path    => '/bin:/usr/bin:/usr/sbin',
      require => Exec['RHSM-register'],
    }
  }
  ...
  if $repo_els_tools {
    exec { 'RHSM-ELS-TOOLS':
      ...
      onlyif  => 'subscription-manager repos --list-disabled | grep server-els-satellite-tools',
      path    => '/bin:/usr/bin:/usr/sbin',
      require => Exec['RHSM-register'],
    }
  }
